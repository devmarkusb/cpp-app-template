...
`MB_CPP_APP_TEMPLATE_HEADER_ONLY=ON` to build the same API as a **header-only** `INTERFACE` library (no object code in
...
prefix `MB_CPP_APP_TEMPLATE`, names `cpp-lib-template`, `mb` / `mb::` / `mb/`, files and dirs). The script exists so you
...
|-----------------------------------------|---------------------|-----------------------------------------------------------------------------|
| `MB_CPP_APP_TEMPLATE_HEADER_ONLY`       | `OFF`               | Header-only `INTERFACE` library vs static library with sources under `src/`. |
| `MB_CPP_APP_TEMPLATE_BUILD_TESTS`       | `ON` when top-level | Build tests and test infra (GoogleTest).                                    |
| `MB_CPP_APP_TEMPLATE_BUILD_EXAMPLES`    | `ON` when top-level | Build example executables.                                                  |
...
- **`mb.cpp-app-template-config.cmake.in`** — Template for the CMake config-file package (used when installing the
  library so consumers can `find_package(mb.cpp-app-template)`).
...
- **`usage.cpp`** — Example program using the library (e.g. `mb::cpp_app_template::sum`).

### `include/`

- **`mb/cpp-app-template/`** — Public headers, exposed via a FILE_SET.

### `src/`

- **`CMakeLists.txt`** — Adds compiled `.cpp` sources to the static library when `MB_CPP_APP_TEMPLATE_HEADER_ONLY` is
  OFF; registers tests when `MB_CPP_APP_TEMPLATE_BUILD_TESTS` is ON.
- **`cpp-app-template.cpp`** — Library implementation (not used in header-only mode).
- **`cpp-app-template.test.cpp`** — GoogleTest sources; built only when `MB_CPP_APP_TEMPLATE_BUILD_TESTS` is ON.

### Root files

- **`CMakeLists.txt`** — Top-level: project, options, library target (static by default, or header-only `INTERFACE`
  when `MB_CPP_APP_TEMPLATE_HEADER_ONLY=ON`), install, tests, examples.
